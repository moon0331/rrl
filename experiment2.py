...
def get_baseball_data_loader(dataset, batch_size, pin_memory=False):
    data_path = os.path.join(DATA_DIR, dataset + '.data')
    info_path = os.path.join(DATA_DIR, dataset + '.info')
    X_df, y_df, f_df, label_pos = read_csv(data_path, info_path, shuffle=False, is_baseball=True)
...
def train_model(gpu, args):
    ...
    if args.data_set == 'baseball_binary':
...
def test_model(args):
    ...
    dataset = args.data_set
    db_enc, train_loader, _, test_loader = get_data_loader(dataset, 4, 0, args.batch_size, args.ith_kfold, save_best=False, valid_ratio=0.9)
